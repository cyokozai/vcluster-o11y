...
> **Note**: `docker build` の `dev` ステージでは `go mod tidy` が、`builder` ステージでは `go build` がコンテナ内で自動実行されます。

マニフェストのイメージ参照を実際の URI に置換します。

```bash
export ECR_BASE="<実際のECR Base URI>"
sed -i "" "s|<ECR_REPO>/go-api-server|${ECR_BASE}/server01|" manifests/pattern-a/deploy.yaml
sed -i "" "s|<ECR_REPO>/go-api-server|${ECR_BASE}/server02|" manifests/pattern-b/deploy.yaml
sed -i "" "s|<ECR_REPO>/go-api-server|${ECR_BASE}/server03|" manifests/pattern-c/deploy.yaml
```
...
#### リクエスト数 (パターン別)
```promql
# Pattern A / B のリクエスト数を一覧
http_server_request_duration_seconds_count{
  service_name=~"go-api-server-pattern-(a|b)"
}

# Pattern C のリクエスト数を一覧
http_requests_total{
  service_name="go-api-server-pattern-c"
}
```
...
# Pattern B のリクエストレート (1分間)
rate(http_server_request_duration_seconds_count{
  service_name="go-api-server-pattern-b"
}[1m])

# Pattern C のリクエストレート (1分間)
